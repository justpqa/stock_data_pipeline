...
dataset_file = "fin_stm_data.csv"
dataset_lst = ["fin_stm_data_" + str(i) + ".csv" for i in range(5)]
dataset_name = "stock_data_all"
BIGQUERY_DATASET = "stock_data_all"
table_name = "fin_stm_data"
...
def get_all_fs(inx):
    if inx >= 0 and inx < 5:
        ...
        data = pd.DataFrame(columns = ["Time", "Ticker", "DividendYield", "TotalCash", "TotalDebt", "TotalRevenue", "RevPerShare", "GrossProfits", "OCF"])
        for i in range(len(company_lst)):
            try:
                info = yf.Ticker(company_lst[i]).info
                data.loc[len(data)] = [datetime.now(), company_lst[i], info['dividendYield'], info['totalCash'], info['totalDebt'], info['totalRevenue'], info["revenuePerShare"], info["grossProfits"], info['operatingCashflow']]
            except:
...
# define the dags
with DAG(
    dag_id="financial_statement_gcs_dag",
    schedule_interval="0 * * * *",
    ...
    for task_inx in range(5):
        task_name = "get_fs_" + str(task_inx)
        new_task = PythonOperator(
            task_id = task_name,
            python_callable = get_all_fs,
            op_kwargs = {
                "inx": task_inx
            },
            dag = dag
        )
        fs_task_lst.append(new_task)
    ...
    gcs_to_bigquery_task = GCSToBigQueryOperator(
        ...
        schema_fields = [
            ...
            {'name': 'TotalCash', 'type': 'FLOAT', 'mode': 'NULLABLE'},
            {'name': 'TotalDebt', 'type': 'FLOAT', 'mode': 'NULLABLE'},
            {'name': 'TotalRevenue', 'type': 'FLOAT', 'mode': 'NULLABLE'},
            {'name': 'RevPerShare', 'type': 'FLOAT', 'mode': 'NULLABLE'},
            {'name': 'GrossProfits', 'type': 'FLOAT', 'mode': 'NULLABLE'},
            {'name': 'OCF', 'type': 'FLOAT', 'mode': 'NULLABLE'}
        ],
        write_disposition='WRITE_APPEND',
        dag=dag,
    )
    
fs_task_lst >> join_all_stocks_task >> local_to_gcs_task >> gcs_to_bigquery_task